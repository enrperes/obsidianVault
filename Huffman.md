### Code con priorità 
Definire una classe `NodeQueue` per sostituire `PriorityQueue<Node>` nel programma `Huffman` con le stesse funzionalità della classe `Node`. 
Realizzare usando Array senza usare classi del package `Java.util`

Successivamente, definire la classe `NodeStack`.
...
NodeQueue mantiene una coda ordinata implicitamente (coda di priotità) (l'array coda non è ordinato, gli elementi vengono aggiunti in fondo e il minimo viene cercato solo quando serve) 
...
Implementa una struttura tipo Stack con le operazioni standard. 
il costruttore inizializza `size` a 0 e l'array `stk` a dimensione 0. 

```java
 
/* 
 * Protocollo di NodeStack
 * 
 * NodeStack ns = new NodeStack() = costruttore
 * ns.empty() = verifica se lo stack è vuoto (bool) 
 * ns.peek() = ritorna l'elemento in cima allo stack senza rimuoverlo (Node)
 * ns.pop() = ritorna l'elemento in cima allo stack e lo rimuove (Node)
 * ns.push(Node n) = aggiunge un nodo in cima allo stack (void) 
 */
...
public class NodeStack{
  ...
  private int size; //size è la posizione dell'ultimo elemento inserito, ovvero la size TEMPORANEA maxSize = 128;
  ...
  public void push( Node n){ // aggiunge un nodo in cima allo stack 
